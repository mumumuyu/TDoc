## Springboot + Shiro 整合JWT

GitHub 项目地址：[https://github.com/Smith-Cruise/Spring-Boot-Shiro](https://links.jianshu.com/go?to=https%3A%2F%2Fgithub.com%2FSmith-Cruise%2FSpring-Boot-Shiro) 

原文地址：[https://www.inlighting.org/archives/spring-boot-shiro-jwt](https://links.jianshu.com/go?to=https%3A%2F%2Fwww.inlighting.org%2Farchives%2Fspring-boot-shiro-jwt)
...
- [JWT](https://jwt.io/) （Json Web Token）的基本概念，并且会简单操作 JWT 的 [JAVA SDK](https://github.com/auth0/java-jwt)。
...
我们写一个简单的 JWT 加密，校验工具，并且使用用户自己的密码充当加密密钥，这样保证了 token 即使被他人截获也无法破解。并且我们在 `token` 中附带了 `username` 信息，并且设置密钥 5 分钟就会过期。
...
| /require_role       | admin 的角色用户才可以登入                     |
| /require_permission | 拥有 view 和 edit 权限的用户才可以访问         |
